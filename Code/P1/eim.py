# - Phase: Empirical Interpolation Method
...
class EIM():
    ...
    def offline(self, train_snap: FunctionsList, Mmax: int, _xm = None, verbose = False):
        ...
        for mm in range(1, Mmax):
            ...
            if verbose:
                print(f'  Iteration {(mm+1)+0:03} | Abs Err: {maxAbsErr[mm-1]:.2e} | Rel Err: {maxRelErr[mm-1]:.2e}', end="\r")
        ...
        if verbose:
            print(f'  Iteration {(mm+1)+0:03} | Abs Err: {maxAbsErr[-1]:.2e} | Rel Err: {maxRelErr[-1]:.2e}', end="\r")

        return maxAbsErr, maxRelErr, beta_coeff

    def reconstruct(self, snap: np.ndarray, Mmax: int, noise_value : float = None):
        r"""
        ...
        .. math::
            y_m = u(\vec{x}_m) +\varepsilon_m \qquad \qquad m = 1, \dots, M
        ...
        y_clean = snap[self.magic_points['idx']]
    ...
    def test_error(self, test_snap: FunctionsList, Mmax: int = None, noise_value : float = None):
        r"""
        ...
        for mu in range(Ns_test):
            beta_coeffs[:, mu], _ = self.reconstruct(test_snap(mu), Mmax, noise_value)
